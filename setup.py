...
#
# However, when we do this, we also import `plic/__init__.py'. If this
...
def git_ls_files(*cmd_args):
    ...
    cmd = ['git', 'ls-files']
    cmd.extend(cmd_args)
    return set(subprocess.check_output(cmd, universal_newlines=True).splitlines())
...
setup_dict = dict(
    ...
    entry_points={
        'console_scripts': [
            'plic = plic.__main__:entry_point'
        ],
        # if you have a gui, use this
        # 'gui_scripts': [
        #     'plic_gui = plic.gui:entry_point'
        # ]
    }
)
